...
fillend:
        movl    d6, obs_value           | only valid for o_&check
        movl    d7, exp_value           | valid for any check.
        movl    sp@+, d6
        movl    sp@+, d7
fillend2:
        unlk    a6
...
1:      movb    d7, a0@+                | stuff into memory
        dbra    d0, 1b                  | decrement until not equal
        dbra    d1, 1b                  | decrement until not equal
...
1:      cmpb    a0@+, d7                | check value
        dbne    d0, 1b
        dbne    d1, 1b                  | loop till value mismatch or done
        jsr     bdb_pack                | construct return
        jra     fillend                 | then finish up

.globl  wcheck, wpcheck
wcheck:
wpcheck:
        jsr     fillstart               | get prams (if needed)
        jsr     wdb_unpack
