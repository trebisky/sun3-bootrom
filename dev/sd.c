...
static int sdopen ( struct saioreq * );
static int sdclose ( struct saioreq * );
static int sdstrategy ( struct saioreq *, int );
 ...
 */
static int
sdspin ( struct saioreq *sip, int dummy )
{
        int     retval;
#ifdef lint
        dummy = dummy;
...
sdopen ( struct saioreq *sip )
{
        ...
        *sdp->subsip = *sip;            /* Initialize sub-sip */
         ...
         */
        switch ( isspinning ( (void *) sdspin, (char *)sip, 0)) {
 ...
 */
static int
sdstrategy ( struct saioreq *sip, int rw )
{
 ...
 */
static int
sdcmd ( int cmd, struct saioreq *sip, int errprint )
{
        int blkno, acount;
        register char *buf;
        register struct sdparam *sdp;
