...
static void iedefaultconf ( struct ieconf *ic );
static void ieca ( struct ie_softc * );
static int iesimple ( struct ie_softc *, struct iecb * );
static int ierustart ( struct ie_softc * );
static int ieinit ( struct saioreq * );

static int iexmit ( struct ie_softc *, char *, int );
...
static ieoff_t to_ieoff ( struct ie_softc *, caddr_t );
static ieaddr_t to_ieaddr ( struct ie_softc *, caddr_t );

struct devinfo ieinfo = {
        0, sizeof (struct ie_softc), 0, 0, (unsigned long *)0, MAP_OBIO,
};

static int ieprobe ( void * );
static int ieopen ( struct saioreq * );
static int ieclose ( struct saioreq * );

/* in sys/xxboot.c */
int tftpboot ( void * );

/* in sys/tftp.c */
int etherstrategy ( void *, int );
int etheropen ( void * );

/* tjt more brute force casting with void * */
 ...
 */
static int
iereset ( struct ie_softc *es, struct saioreq *sip )
{
 ...
 */
static int
iexmit ( struct ie_softc *es, char *buf, int count )
{
...
static int
iepoll ( struct ie_softc *es, char *buf )
{
        register struct ierbd *rbd = &es->es_rbd;
        register struct ierfd *rfd = &es->es_rfd;
        register struct iescb *scb = &es->es_scb;
 ...
 */
static ieaddr_t
to_ieaddr ( struct ie_softc *es, caddr_t cp )
{
        union {
                int     n;
                char    c[4];
 ...
 */
static ieoff_t
to_ieoff ( struct ie_softc *es, caddr_t addr )
{
        union {
                short   s;
                char    c[2];
 ...
 */
static void
iedefaultconf ( struct ieconf *ic )
{
