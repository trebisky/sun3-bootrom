...
static int scopen ( struct saioreq * );

static int sc_wait ( struct scsi_ha_reg *, int );
static int sc_putbyte ( register struct scsi_ha_reg *, int, int );
static int sc_getbyte ( register struct scsi_ha_reg *, int );
...
static char seqerr_msg[] = "sequence error";   /* Saves dup copy of string */
 ...
 */
int
scdoit ( struct scsi_cdb *cdb, struct scsi_scb *scb, struct saioreq *sip )
{
        register char *cp;
        register int i, b;
        register struct scsi_ha_reg *har;
 ...
 */
static int
sc_wait ( struct scsi_ha_reg *har, int cond )
{
 ...
 */
static int
sc_putbyte ( register struct scsi_ha_reg *har, int  bits, int c )
{
 ...
 */
static int
sc_getbyte ( register struct scsi_ha_reg *har, int  bits )
{
        if (sc_wait(har, ICR_REQUEST) == 0) {
                return (-1);
        }
