...
static void drawchr ( short *, int );
static void inschar ( unsigned char, unsigned char, unsigned char );
static void delchar ( unsigned char, unsigned char, unsigned char );
static void insline ( unsigned char, unsigned char );
static void delline ( unsigned char, unsigned char );
static void invertwholescreen ( void );
static void rectfill ( int, int, int, int );
...
/* tjt - this fails with gcc, it ignores the argument and
 * always uses the character 'c'.  No doubt there is a way
 * to rewrite this and make it work, but I took a different
 * approach.
...
void
fwritestr ( unsigned char *addr, int len )
...
beginning:
        ...
        if (state & ESC) {
                switch (c) {
                        ...
                        case CTRL_J:       /*ignored*/
                        case CTRL_M:       /*ignored*/
                        case CTRL_BRACK:   /*ignored*/
                        case CTRL_QUE:     /*ignored*/             continue;
        ...
        case ALPHA:
                ...
                ac0 = -1;
                switch (c) {
                        case CTRL_G:   blinkscreen();          break;
                        case CTRL_H:   pos(ax-1,ay);           break;
                        case CTRL_I:   pos((ax&-8)+8,ay);      break;
                        case CTRL_J:   /* linefeed */
...
FeedLine:
        ...
        break;
                        case CTRL_K:   pos(ax,ay-1); /* 4014 */        break;
                        case CTRL_L:   pos(LEFT,TOP);
                                        rectfill(0,0,SCRWIDTH,SCRHEIGHT);
#ifdef PRISM
                                        rect_enable(0,0,SCRWIDTH,SCRHEIGHT,
                                                    0xFFFF);
                                                /* enable the mono FB */
#endif PRISM
                        case CTRL_M:   /* pos(0,ay); */
                                        ax = 0;
                                        fbpos.pos.x = WINLEFT;
                                        break;
                        case CTRL_BRACK:   state |= ESC;           break;
                        case CTRL_QUE:   /* ignored */           break;
...
/* The 2nd param (font) is used as the source for the character sent to the
   ...
   so we just punt with  font .         */
                drawchr( (short *) font, POX_NOT(POX_DST));
}
 ...
 */
void
rectcopy ( int fromxlo, int fromylo, int fromxhi, int fromyhi, int tox,int toy )
{
        int y, yy, toyy;
        int width, height;
 ...
 */
static void
rectfill ( int xlo, int ylo, int xhi, int yhi )
{
        int yy, width, filler;
        unsigned short y;
...
static void
delline ( unsigned char a, unsigned char b )
{
        register int pixla = a*CHRHEIGHT;
		register int pixlb = b*CHRHEIGHT;
...
static void
insline ( unsigned char a, unsigned char b )
{
        register int pixla = a*CHRHEIGHT;
		register int pixlb = b*CHRHEIGHT;
 ...
 */
static void
delchar ( unsigned char a,  unsigned char b,  unsigned char l )
{
        int savex = ax;
        int savey = ay;
        ...
        drawchr ( (short *) font, fillfunc);        /* Fill one char position there */
...
static void
inschar ( unsigned char a, unsigned char b, unsigned char l )
{
        int savex = ax;
        int savey = ay;
        register int apos, bpos;
        ...
        for (; a < b; a++, fbpos.pos.x += CHRWIDTH) {
                drawchr ( (short *) font, fillfunc);        /* Fill one char position */
        }
