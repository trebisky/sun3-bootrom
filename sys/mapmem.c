...
mapmem( unsigned long memsize )
{
        ...
        setpgmap ( (char *) MONBSS_BASE, 0xd0000000 + (memsize >> 13) - 2);
 ...
 *
 * also called from reset.c
 */
void
setupmap ( struct pginit table[] )
{
        ...
        for (k = table; k->pi_incr != PGINITEND; k++) {
                ...
                if (k->pi_incr != PGINITSKIP) {
                        for (; i < k[1].pi_addr;
                               i += BYTESPERPG, pg.u_pgmap.pm_page += k->pi_incr) {
                                setpgmap ( i, pg.u_val );
                        }
                }
        }       /* end of for (k=table; ... ) */
